...
@blueprint.route('/sysdev-detail-<devname>.html', methods = ['GET', 'POST'])
@login_required
@cache.cached(timeout=6)
def route_sysdev_detail(devname):
        ...
        if not (pgdef is None or define is None):
            for etfield in pgdef['pagedef']['et_fields']:
                if etfield['type'] == 'jsoneditor':
                    ...
                    break
            define['pagedef'] = pgdef
            return render_template('sysdev/sysdev-detail.html', segment='infra-' + devname, nav=nav,
